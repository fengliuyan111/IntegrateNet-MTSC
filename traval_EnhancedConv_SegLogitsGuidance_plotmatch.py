...
from error import *
from IntegrateNet_EnhancedConv_SegLogitsGuidance import *
...
def main(lamda, seg_weight=0.0, eval_with_mask=True):
    ## random seed
    ...
    print("="*60)
    print(f"Model: IntegrateNet - EnhancedConv + SegLogitsGuidance")
    print(f"  - Enhanced DoubleConv (Residual + CBAM)")
    print(f"  - Seg-Logits Guidance Mechanism")
    ...
    for epoch in range(num_epoch):
        ...
        if epoch % 100 == 99:
            save_checkpoint(state,"results/models/IntegrateNet_EnhancedConv_SegLogitsGuidance", filename='model_ckpt'+str(epoch)+'.pth.tar')

        # save best model parameters and results
        if net.measure['plots_mae'][-1] <= best_mae:
            save_checkpoint(state, "results/models/IntegrateNet_EnhancedConv_SegLogitsGuidance", filename='model_best.pth')
    ...
    plt.savefig("Loss_seg_EnhancedConv_SegLogitsGuidance.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    main(0.5, seg_weight=1.0, eval_with_mask=True)
