...
from error import *
from IntegrateNet_DS_SegGuidance import *
...
def main(lamda, seg_weight=0.0, eval_with_mask=True, resume=True, resume_path=None):
    ...
    print("="*60)
    print(f"Model: IntegrateNet - DS + SegGuidance")
    ...
    if resume:
        model_dir = "results/models/IntegrateNet_DS_SegGuidance"
    ...
    for epoch in range(start_epoch, num_epoch):
        ...
        if epoch % 100 == 99:
            save_checkpoint(state,"results/models/IntegrateNet_DS_SegGuidance", filename='model_ckpt'+str(epoch)+'.pth.tar')

        # save best model parameters and results
        if net.measure['plots_mae'][-1] <= best_mae:
            save_checkpoint(state, "results/models/IntegrateNet_DS_SegGuidance", filename='model_best.pth')
    ...
    plt.savefig("Loss_DS_SegGuidance.png", dpi=300)
    plt.close()
