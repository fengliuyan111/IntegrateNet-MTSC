...
from error import *
from IntegrateNet_EnhancedConv_DS_SegGuidance import *
...
def train(net, train_loader, optimizer, criterion, criterion2, epoch, lamda, seg_weight=0.0, bce_loss=None):
    ...
    for i, sample in enumerate(train_loader):
        ...
        for j in range(length):
            ...
            # 主损失
            loss1 = criterion(density, target)  # MSE
            target_count = F.conv2d(target, target_filter, stride=64)  # 生成局部计数图
            loss2 = criterion2(local_count, target_count)  # L1
    ...
    # if epoch % 10 == 0:
    #     print(f'Epoch {epoch}: total_loss={running_loss/(length*(i+1)):.5f}, '
    #           f'main_loss={running_main_loss/(length*(i+1)):.5f}, '
    #           f'aux_loss={running_aux_loss/(length*(i+1)):.5f}')
...
def main(lamda, seg_weight=0.0, eval_with_mask=True, resume=True, resume_path=None):
    ...
    Args:
        lamda: 损失函数权重
        seg_weight: 分割损失权重
        eval_with_mask: 是否使用mask评估
        resume: 是否尝试恢复训练（默认True）
        resume_path: 指定checkpoint路径，如果为None则自动查找最新的
    """
    ## random seed
    ...
    # Initiate network with deep supervision and seg-logits guidance
    ...
    print("="*60)
    print(f"Model: IntegrateNet - EnhancedConv + DS + SegGuidance")
    print(f"  - Enhanced DoubleConv (Residual + CBAM)")
    print(f"  - Deep Supervision at 1/16 and 1/8 scales")
    print(f"  - Seg-Logits Guidance (spatial attention)")
    ...
    if resume:
        model_dir = "results/models/IntegrateNet_EnhancedConv_DS_SegGuidance"
        
        # 如果没有指定路径，自动查找最新的checkpoint
        if resume_path is None:
            # 首先查找model_latest.pth.tar（每10个epoch保存的）
            latest_path = os.path.join(model_dir, 'model_latest.pth.tar')
            if os.path.exists(latest_path):
                resume_path = latest_path
                print(f"Found latest checkpoint: {resume_path}")
            # 如果没有，查找最近的定期保存checkpoint（model_ckpt99.pth.tar, model_ckpt199.pth.tar等）
        ...
        if resume_path and os.path.exists(resume_path):
            ...
            if history is not None:
                ...
                if len(net.measure.get('plots_mae', [])) > 0:
                    ...
                    print(f"Current best MAE: {best_mae:.2f} at epoch {best_idx}")
        else:
    ...
    for epoch in range(start_epoch, num_epoch):
        ...
        if epoch % 100 == 99:
            save_checkpoint(state,"results/models/IntegrateNet_EnhancedConv_DS_SegGuidance", filename='model_ckpt'+str(epoch)+'.pth.tar')

        # save best model parameters and results
        if net.measure['plots_mae'][-1] <= best_mae:
            save_checkpoint(state, "results/models/IntegrateNet_EnhancedConv_DS_SegGuidance", filename='model_best.pth')
    ...
    plt.savefig("Loss_EnhancedConv_DS_SegGuidance.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    # resume=True: 自动恢复最近的checkpoint继续训练
    # resume=False: 从头开始训练
    # resume_path="xxx.pth.tar": 从指定checkpoint恢复
