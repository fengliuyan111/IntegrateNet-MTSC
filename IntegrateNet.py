...
class IntegrateNet(nn.Module):
    def __init__(self):
        ...
        # 方案1：通道拼接（原实现）
        # self.Doubleconv1 = DoubleConv(in_channels=256,out_channels=128)  # 256=128+128
        # self.Doubleconv2 = DoubleConv(in_channels=128,out_channels=64)   # 128=64+64

        # 方案2：逐元素相加（论文实现）
        self.Doubleconv1 = DoubleConv(in_channels=128,out_channels=128)  # 保持128通道
        self.Doubleconv2 = DoubleConv(in_channels=64,out_channels=64)    # 保持64通道
    ...
    def forward(self, output):
        ...
        # 方案1：通道拼接（原实现）
        # output = torch.cat((one_16,output),dim=1)  # 128+128=256通道
        # output = self.Doubleconv1(output)  # 256->128通道
        ...
        # 方案1：通道拼接（原实现）
        # output = torch.cat((one_8,output),dim=1)  # 64+64=128通道
        # output = self.Doubleconv2(output)  # 128->64通道
