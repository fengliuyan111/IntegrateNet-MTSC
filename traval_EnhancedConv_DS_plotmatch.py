...
from error import *
from IntegrateNet_EnhancedConv_DS import *
...
def main(lamda, seg_weight=0.0, eval_with_mask=True, resume=True, resume_path=None):
    ...
    # Initiate network with deep supervision
    ...
    print("="*60)
    print(f"Model: IntegrateNet - EnhancedConv + Deep Supervision")
    ...
    if resume:
        model_dir = "results/models/IntegrateNet_EnhancedConv_DS"
    ...
    for epoch in range(start_epoch, num_epoch):
        ...
        if epoch % 100 == 99:
            save_checkpoint(state,"results/models/IntegrateNet_EnhancedConv_DS", filename='model_ckpt'+str(epoch)+'.pth.tar')

        # save best model parameters and results
        if net.measure['plots_mae'][-1] <= best_mae:
            save_checkpoint(state, "results/models/IntegrateNet_EnhancedConv_DS", filename='model_best.pth')
    ...
    plt.savefig("Loss_EnhancedConv_DS.png", dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    main(0.5, seg_weight=1.0, eval_with_mask=True, resume=True)
